...
class EventItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var lunarDates = convertSolar2Lunar(event.date.day, event.date.month, event.date.year, 7);

    var lunarDay = lunarDates[0];
    var lunarMonth = lunarDates[1];
    var title = '* ${dayOfWeek} - ${event.date.day}/${event.date.month} (${lunarDay}/${lunarMonth} AL)';
    return Center(
      child: Column(
